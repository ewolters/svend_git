...
from .doe import ExperimentDesign
from .stats import PowerAnalyzer
    ...
    **kwargs,
    ...
    if sample_sizes:
        ...
        for i, power in enumerate(powers):
            if power >= 0.8:
                ax.axvline(x=sample_sizes[i], color="gray", linestyle=":", label=f"n={sample_sizes[i]} for 80% power")
                break
...
def plot_sample_size_curve(
    ...
    for i, power in enumerate(power_levels):
        ...
        ax.plot(effect_sizes, sample_sizes, color=colors[i % len(colors)], linewidth=2, label=f"Power = {power:.0%}")
...
def plot_design_matrix(design: ExperimentDesign, coded: bool = True, output_path: Path = None) -> str:
    """
    ...
    if coded and n_runs <= 32:
        for i in range(n_runs):
            for j in range(n_factors):
                ...
                ax.text(j, i, f"{val:+.0f}" if val != 0 else "0", ha="center", va="center", color=color, fontsize=9)
...
def plot_factor_effects(
    design: ExperimentDesign, response_name: str = "Response", responses: list[float] = None, output_path: Path = None
) -> str:
    """
...
def plot_interaction(
    ...
    for j, level2 in enumerate(f2.levels):
        ...
        for level1 in f1.levels:
            mask = [r.factor_levels[factor1] == level1 and r.factor_levels[factor2] == level2 for r in design.runs]
...
def plot_residuals(predicted: list[float], actual: list[float], output_path: Path = None) -> str:
    """
