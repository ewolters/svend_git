...
from .doe import DOEGenerator, ExperimentDesign, Factor
from .plots import plot_design_matrix, plot_power_curve
from .stats import PowerAnalyzer, PowerResult, SampleSizeResult, interpret_effect_size
...
class ExperimenterAgent:
    ...
    def _generate_interpretation(self, result: ExperimentResult):
        ...
        if result.power_result:
            ...
            if pr.power < 0.8:
                warnings.append(
                    f"Power ({pr.power:.0%}) is below the conventional 80% threshold. Consider increasing sample size."
                )
        ...
        if result.design:
            ...
            if design.num_runs < total_combinations:
                ...
                result.summary += f" This tests all {total_combinations} factor combinations."
            ...
            if design.num_runs > 64:
                recommendations.append(
                    f"Consider a fractional factorial or screening design to reduce runs from {design.num_runs}."
                )

            if design.num_center_points > 0:
                recommendations.append(
                    "Center points included. These allow detection of curvature (non-linear effects) in the response."
                )

            # Always recommend randomization
            recommendations.append(
                "Run experiments in the randomized order shown to minimize bias from time-related factors."
            )
...
def quick_power(effect_size: float, test: str = "ttest", alpha: float = 0.05, power: float = 0.80) -> PowerResult:
...
def quick_factorial(factors: dict[str, list], seed: int = None) -> ExperimentDesign:
    ...
    factor_list = [{"name": name, "levels": levels} for name, levels in factors.items()]
    request = ExperimentRequest(
