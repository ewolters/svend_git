                ...
                if not result["is_valid"]:
                    ...
                    for violation in result["violations"]:
                        try:
                            ...
                            logger.critical(f"Audit integrity violation for tenant {tenant_id}: {violation['type']}")
...
def cleanup_old_violations(self, days: int = 90):
    ...
    try:
        ...
        logger.info(f"Cleaned up {deleted_count} resolved integrity violations older than {days} days")
