...
class CrossFMEAPatternsTest(TestCase):
    ...
    def setUp(self):
        ...
        _post(self.client, f"/api/fmea/{self.fmea_a}/rows/", {
        ...
        _post(self.client, f"/api/fmea/{self.fmea_b}/rows/", {
...
class RCAClusteringTest(TestCase):
    ...
    def test_cluster_excludes_drafts(self):
        ...
        RCASession.objects.create(
            owner=self.user, title="Draft", event="Test",
            status="draft", embedding=emb.tobytes(),
        )
        resp = _post(self.client, "/api/rca/clusters/", {})
        self.assertEqual(resp.status_code, 200)
...
class RCAGuidedQuestionsTest(TestCase):
    ...
    def test_guided_questions_returns_structured(self, mock_anthropic_cls):
        ...
        resp = _post(self.client, "/api/rca/guided-questions/", {
...
class A3CritiqueTest(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create(
            user=self.user, title="A3 Critique Project",
        )
    ...
    def test_critique_empty_report(self):
        ...
        empty_report = A3Report.objects.create(
            owner=self.user, project=self.project, title="Empty",
        )
...
class VSMWasteAnalysisTest(TestCase):
    ...
    def setUp(self):
        ...
        self.vsm = ValueStreamMap.objects.create(
            ...
            process_steps=[
                {"id": "1", "name": "CNC", "cycle_time": 45, "changeover_time": 1800,
                 "uptime": 72, "batch_size": 200, "operators": 1},
                {"id": "2", "name": "Assembly", "cycle_time": 30, "changeover_time": 60,
                 "uptime": 95, "batch_size": 10, "operators": 2},
            ],
            inventory=[
    ...
    def test_timwoods_classification(self):
        ...
        for cat in ["transport", "inventory", "motion", "waiting",
...
class HoshinAlignmentTest(TestCase):
    ...
    def setUp(self):
        ...
        self.site = Site.objects.create(
            tenant=self.tenant, name="Plant A",
        )
    ...
    def test_unlinked_projects_detected(self):
        ...
        core_project = Project.objects.create(
            tenant=self.tenant, title="Unlinked Project",
        )
        HoshinProject.objects.create(
            project=core_project, site=self.site,
...
class QMSHealthDashboardTest(TestCase):
    ...
    def test_dashboard_aggregates_all_modules(self):
        ...
        RCASession.objects.create(
            owner=self.user, title="Test RCA", event="Test", status="investigating",
        )
...
class SPCFMEATriggerTest(TestCase):
    ...
    def test_ooc_triggers_fmea_update(self):
        ...
        resp = _post(self.client, "/api/spc/chart/", {
            "chart_type": "I-MR",
            "data": data,
...
class SPCEvidenceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.force_login(self.user)
        self.project = Project.objects.create(
            user=self.user, title="SPC Evidence Project",
        )
