...
def _run_inventory_policy_wizard(df, config):
    ...
    for _, row in work.iterrows():
        ...
        # Kanban cards
        kanban_cards = math.ceil(
            (demand * lead_time * (1 + ss / max(demand * lead_time, 1)))
            / max(container_size, 1)
        )
        ...
        if cls_items:
            class_summary[cls] = {
                ...
                "avg_safety_stock": round(
                    np.mean([p["safety_stock"] for p in cls_items]), 1
                ),
                ...
                "avg_reorder_point": round(
                    np.mean([p["reorder_point"] for p in cls_items]), 1
                ),
            }
