...
def dsw_from_intent(request):
    ...
    try:
        ...
        belief_result = _bayesian_model_beliefs(
            metrics, X, y, importances, task, model=model
        )
            ...
            "warnings": [
                {
                    "level": (
                        ...
                        else "warning" if b["probability"] > 0.4 else "info"
                    ),
                    "msg": b["narrative"],
                }
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if file_type == "report":
                ...
                return FileResponse(
                    open(path, "rb"), as_attachment=True, filename="report.md"
                )
                ...
                return FileResponse(
                    open(path, "rb"), as_attachment=True, filename="train.py"
                )
                ...
                if data_path and Path(data_path).exists():
                    return FileResponse(
                        open(data_path, "rb"), as_attachment=True, filename="data.csv"
                    )
                else:
                ...
                try:
                    saved_model = SavedModel.objects.get(
                        dsw_result_id=result_id, user=request.user
                    )
                    ...
                    if model_path.exists():
                        ...
                        return JsonResponse(
                            {"error": "Model file not found"}, status=404
                        )
                except SavedModel.DoesNotExist:
                    return JsonResponse(
                        {"error": "No saved model for this result"}, status=404
                    )
            else:
                return JsonResponse(
                    {"error": f"Unknown file type: {file_type}"}, status=400
                )
            ...
            "models": [
                {
                    ...
                    "features": (
                        json.loads(m.feature_names) if m.feature_names else None
                    ),
                    ...
                    "version": m.version,
                    "parent_model_id": (
                        str(m.parent_model_id) if m.parent_model_id else None
                    ),
                }
                for m in models_qs
            ]
...
def save_model_from_cache(request):
    ...
    try:
        ...
        if not cached:
            return JsonResponse(
                {"error": "Model not found in cache. It may have expired."}, status=404
            )
...
def run_model(request, model_id):
    ...
    try:
        ...
        features = (
            json.loads(saved_model.feature_names)
            if saved_model.feature_names
        ...
        if conformal_state:
            try:
                ...
                if conformal_state["type"] == "regression":
                    ...
                    intervals = [
                        [round(float(lo[i]), 4), round(float(hi[i]), 4)]
                        for i in range(len(lo))
                    ]
                elif (
                    conformal_state["type"] == "classification"
...
def optimize_model(request, model_id):
    ...
    try:
        ...
        features = (
            json.loads(saved_model.feature_names) if saved_model.feature_names else []
        )
        training_config = saved_model.training_config or {}
        feature_info = training_config.get("feature_info", {})
        ...
        if not numeric_feats:
            return JsonResponse(
                {"error": "No numeric features to optimize"}, status=400
            )

        num_idx = {f: i for i, f in enumerate(numeric_feats)}
        current_numeric = np.array(
            [
                float(current_values.get(f, (bounds[i][0] + bounds[i][1]) / 2))
        ...
        has_costs = any(
            cost_weights.get(f) and float(cost_weights[f]) != 1 for f in numeric_feats
        )

        cov_inv, means_arr, density_threshold = None, None, 1.0
        if (
            use_density
            and feature_stats.get("covariance")
            and feature_stats.get("means")
        ):
            try:
                stat_feats = feature_stats["numeric_features"]
                means_arr = np.array(
                    [feature_stats["means"].get(f, 0) for f in stat_feats]
                )
                cov_matrix = np.array(
                    [
                        [feature_stats["covariance"][c][r] for c in stat_feats]
                        for r in stat_feats
                    ]
                )
        ...
        for cat_vals in cat_combos:
            ...
            def objective(x, _cv=cat_vals):
                ...
                obj = (
                    -pred
                    if goal == "maximize"
                    else (pred if goal == "minimize" else abs(pred - target_value))
                )
                if cov_inv is not None:
                    sf = feature_stats["numeric_features"]
                    x_full = np.array(
                        [
                            float(x[num_idx[f]]) if f in num_idx else means_arr[i]
                    ...
                    if mahal > density_threshold:
                        obj += (
                            0.05
                            * pred_scale
                            * ((mahal - density_threshold) / density_threshold) ** 2
                        )
                if has_costs:
                ...
                for sc in sum_constraints:
                    ...
                    feat_sum = sum(
                        float(x[num_idx[f]]) for f in sc_feats if f in num_idx
                    )
                    if sc_op == "<=" and feat_sum > sc_limit:
                        obj += (
                            10
                            * pred_scale
                            * ((feat_sum - sc_limit) / max(sc_limit, 1)) ** 2
                        )
                    elif sc_op == ">=" and feat_sum < sc_limit:
                        obj += (
                            10
                            * pred_scale
                            * ((sc_limit - feat_sum) / max(sc_limit, 1)) ** 2
                        )
                return obj
            ...
            if (
                ...
                or (
                    goal == "target"
                    and abs(pred - target_value) < abs(best_val - target_value)
                )
            ):
                best_val = pred
                best_inputs = {}
        ...
        if cov_inv is not None:
            try:
                ...
                opt_vals = np.array(
                    [
                        float(best_inputs.get(f, feature_stats["means"].get(f, 0)))
                        for f in stat_feats
                    ]
                )
                ...
                if mahal_dist <= threshold_ok:
                    ...
                    feasibility = {
                        "score": round(
                            0.5 - (mahal_dist - threshold_ok) / (threshold_warn * 2), 2
                        ),
                        "label": "moderate",
                        "mahalanobis": round(mahal_dist, 2),
                    ...
                    feasibility = {
                        "score": max(
                            0.0, round(0.2 - mahal_dist / (threshold_warn * 5), 2)
                        ),
                        "label": "low",
                        "mahalanobis": round(mahal_dist, 2),
                ...
                for corr in feature_stats.get("strong_correlations", []):
                    ...
                    if f1 in best_inputs and f2 in best_inputs:
                        v1 = (
                            float(best_inputs[f1]) - feature_stats["means"].get(f1, 0)
                        ) / max(feature_stats["stds"].get(f1, 1), 1e-6)
                        v2 = (
                            float(best_inputs[f2]) - feature_stats["means"].get(f2, 0)
                        ) / max(feature_stats["stds"].get(f2, 1), 1e-6)
                        if r > 0 and v1 * v2 < -1:
                            violated.append(
        ...
        for i, feat in enumerate(numeric_feats):
            ...
            pred_up, pred_down = _predict_row(x_up, best_cat), _predict_row(
                x_down, best_cat
            )
        ...
        for i, feat in enumerate(numeric_feats):
            ...
            preds = [
                _predict_row(
                    [sv if j == i else best_num[j] for j in range(len(numeric_feats))],
                    best_cat,
                )
                for sv in steps
            ]
        ...
        for sc in sum_constraints:
            ...
            feat_sum = sum(
                float(best_inputs.get(f, 0)) for f in sc_feats if f in best_inputs
            )
        ...
        if hasattr(model, "estimators_"):
            try:
                opt_row = {
                    f: best_inputs.get(f, current_values.get(f, "")) for f in features
                }
                ...
                for col in opt_df.select_dtypes(include=["object", "category"]).columns:
                    if col in feature_info and feature_info[col].get("categories"):
                        opt_df[col] = pd.Categorical(
                            opt_df[col],
                            categories=sorted(feature_info[col]["categories"]),
                        ).codes.astype(int)
                    else:
                ...
                X_opt = (
                    opt_df[features]
                    if all(f in opt_df.columns for f in features)
                    else opt_df
                )
    ...
    for col in df.select_dtypes(include=["object", "category"]).columns:
        if col in feature_info and feature_info[col].get("categories"):
            df[col] = pd.Categorical(
                df[col], categories=sorted(feature_info[col]["categories"])
            ).codes.astype(int)
        else:
...
def model_report(request, model_id):
    ...
    if not model.dsw_result_id:
        return JsonResponse(
            {"error": "No training report stored for this model"}, status=404
        )
        ...
        return JsonResponse(
            {
                ...
                "normalizations": (
                    result.normalization.total_changes if result.normalization else 0
                ),
