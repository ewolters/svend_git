...
def run_reliability_analysis(df, analysis_id, config):
    ...
    if analysis_id == "weibull":
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "xaxis": {"title": "ln(Time)"}, "yaxis": {"title": "ln(-ln(1-F))"}, "showlegend": True}
        })
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "xaxis": {"title": "Time"}, "yaxis": {"title": "Reliability R(t)", "range": [0, 1]}}
        })
        ...
        # Narrative
        _wb_phase = "infant mortality (decreasing failure rate)" if shape < 1 else ("random failures (constant rate)" if abs(shape - 1) < 0.1 else "wear-out (increasing failure rate)")
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "xaxis": {"title": "ln(Time)"}, "yaxis": {"title": "Std Normal Quantile"}, "showlegend": True}
        })
        ...
        result["guide_observation"] = f"Lognormal: \u03bc={mu:.3f}, \u03c3={sigma:.3f}. B10={b10:.2f}, median={b50:.2f}."
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "xaxis": {"title": "Time"}, "yaxis": {"title": "-ln(1-F)"}, "showlegend": True}
        })
        ...
        result["guide_observation"] = f"Exponential: \u03bb={rate:.6f}, MTTF={mttf:.2f} (95% CI: {mttf_lower:.2f}-{mttf_upper:.2f})."
        ...
        if len(km_times) > 1:
            ...
            for _nt in _nar_ticks:
                ...
                _nar_annotations.append({
                    "x": float(_nt), "y": -0.08, "yref": "paper",
                    "text": str(_nar), "showarrow": False,
                    "font": {"size": 10, "color": "#7A8F7A"}
                })
            # Label
            _nar_annotations.append({
                "x": float(_t_min) - (_t_max - _t_min) * 0.05, "y": -0.08, "yref": "paper",
                "text": "At risk:", "showarrow": False,
                "font": {"size": 10, "color": "#7A8F7A", "weight": "bold"}, "xanchor": "right"
            })
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "margin": {"b": 60}, "xaxis": {"title": "Time"}, "yaxis": {"title": "Survival Probability", "range": [0, 1.05]}, "showlegend": True, "annotations": _nar_annotations}
        })
        ...
        result["guide_observation"] = f"Kaplan-Meier: {n_events} events, {n_censored} censored. Median survival: {median_survival}."
        ...
        result["narrative"] = _narrative(
            ...
            + (f" Median survival time = {median_survival}." if median_survival != "N/A" else " Median not reached — more than 50% survived the observation period."),
        ...
        if dist == "exponential":
            ...
            result["summary"] = summary
            result["guide_observation"] = f"Reliability test plan: target R={target_rel*100:.1f}% at {confidence*100:.0f}% confidence. Zero-failure plan: n={n_required}."
            result["narrative"] = _narrative(
                ...
                f"Allowing 1 failure increases n to {results_table[1]['sample_size']}." if len(results_table) > 1 else "",
            ...
            result["guide_observation"] = f"Weibull test plan: target R={target_rel*100:.1f}%, \u03b2={beta}, n={n_required}."
            result["narrative"] = _narrative(
        ...
        result["summary"] = summary
        result["guide_observation"] = f"Distribution ID: best fit = {best_name} (KS p={best_info['ks_p']:.4f}). {n} data points."
        ...
        result["narrative"] = _narrative(
            ...
            + (f"Good fit (p > 0.05). " if _di_good_fit else f"Marginal fit (p < 0.05) — consider transforming data or mixed distributions. ")
        ...
        result["plots"].append({
            ...
            "data": hist_traces,
            "layout": {"height": 340, "showlegend": True, "xaxis": {"title": "Time"}, "yaxis": {"title": "Density"}}
        })
        ...
        result["summary"] = summary
        result["guide_observation"] = f"ALT ({model_type}): R\u00b2={r_value**2:.3f}. At use stress ({use_stress}): B10={b10_use:.1f}, MTTF={mttf_use:.1f}."
        ...
        result["plots"].append({
            ...
            "layout": {"height": 340, "xaxis": {"title": "Time"}, "yaxis": {"title": "Reliability", "range": [0, 1.05]}}
        })
        ...
        result["summary"] = summary
        result["guide_observation"] = f"Crow-AMSAA: \u03b2={beta_crow:.3f}, \u03b8={theta_crow:.1f}. {'Deteriorating' if beta_crow > 1 else ('Improving' if beta_crow < 1 else 'Stable')} (Laplace p={laplace_p:.4f})."

        # Narrative
        _rs_trend = "deteriorating (failure rate increasing)" if beta_crow > 1 else ("improving (reliability growth)" if beta_crow < 1 else "stable (constant failure rate)")
        ...
        result["guide_observation"] = f"Warranty: {projected_in_warranty:.0f} projected returns ({_wr_pct:.2f}%) from {fleet_size} fleet. Weibull \u03b2={shape:.3f}."
