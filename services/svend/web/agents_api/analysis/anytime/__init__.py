...
def _run_ab_test(df, config, result):
    ...
    if len(vals_a) < 5 or len(vals_b) < 5:
        result["summary"] = (
            f"Error: Need at least 5 observations per group. Got {len(vals_a)} (A) and {len(vals_b)} (B)."
        )
        return result
    ...
    lines.append("  1. Observations are independent within and between groups")
    lines.append("  2. Data is approximately Gaussian (or at least sub-Gaussian)")
    lines.append("  3. Observations are paired FIFO (first A with first B, etc.)")
    lines.append(f"  4. The mixing prior ρ={rho} reflects your expected effect size")
    lines.append("     (larger ρ → more sensitive to large effects, less to small)")
    ...
    result["guide_observation"] = (
        ...
        + (
            "REJECT — groups differ significantly. Safe to stop."
            if rejected
            else "CONTINUE — no significant difference yet. Keep collecting or stop without guilt."
        )
    )
...
def _run_one_sample(df, config, result):
    ...
    lines = []
    lines.append(f"<<COLOR:accent>>{'═' * 70}<</COLOR>>")
    lines.append("<<COLOR:title>>ANYTIME-VALID ONE-SAMPLE TEST<</COLOR>>")
    lines.append(f"<<COLOR:accent>>{'═' * 70}<</COLOR>>\n")
    ...
    lines.append("<<COLOR:text>>Valid at every sample size simultaneously.<</COLOR>>")
    ...
    result["guide_observation"] = (
        ...
        f"CS=[{L:.4f}, {U:.4f}]. "
        + (
            "REJECT — mean differs from " + str(mu0) + "."
            if rejected
            else "CONTINUE — no evidence against μ=" + str(mu0) + "."
        )
    )
