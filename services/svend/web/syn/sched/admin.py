...
from .types import TaskState, TaskPriority, QueueType
...
class DeadLetterEntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Task Information', {
            'fields': ('id', 'original_task', 'task_id_display', 'task_name_display', 'tenant_id', 'correlation_id_display')
        }),
        ('Status', {
    ...
    def mark_retried(self, request, queryset):
        ...
        for entry in queryset.filter(status='pending'):
            ...
            from .core import CognitiveScheduler
