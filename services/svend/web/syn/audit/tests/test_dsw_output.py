...
def _assert_quality_output(test_case, result, label, require_education=True):
    ...
    if result.get("narrative") is not None:
        test_case.assertIsInstance(result["narrative"], dict,
    ...
    for plot in result.get("plots", []):
        if isinstance(plot, dict) and "layout" in plot:
            test_case.assertIn("height", plot["layout"],
                               f"{label}: chart missing height")
            test_case.assertEqual(plot["layout"].get("paper_bgcolor"), "rgba(0,0,0,0)",
                                  f"{label}: chart missing transparent bg")
    # Education present (when required)
    if require_education:
        test_case.assertIsNotNone(result.get("education"),
                                   f"{label}: missing education")
    ...
    def test_field_types_correct(self):
        ...
        if result["narrative"] is not None:
            self.assertIsInstance(result["narrative"], dict,
                                 "narrative must be dict, not str")
    ...
    def test_conditional_what_if(self):
        ...
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            if entry.get("what_if_tier", 0) >= 1:
                result = standardize_output(
                    {"summary": "test"}, atype, aid)
                self.assertIsNotNone(result.get("what_if"),
                                     f"what_if missing for tier {entry['what_if_tier']} "
                                     f"analysis {atype}/{aid}")
...
class NarrativeQualityTest(SimpleTestCase):
    ...
    def test_string_narrative_normalized(self):
        ...
        self.assertIsInstance(result["narrative"], dict,
                              "String narrative not normalized to dict")
    ...
    def test_verdict_minimum_length(self):
        ...
        self.assertGreaterEqual(len(verdict), 10,
                                f"Verdict too short: '{verdict}' ({len(verdict)} chars)")
    ...
    def test_verdict_no_color_tags(self):
        ...
        verdict = result["narrative"]["verdict"]
        self.assertNotIn("<<COLOR:", verdict,
                         f"Color tags in verdict: {verdict}")
    ...
    def test_guide_observation_bounds(self):
        ...
        self.assertGreaterEqual(len(obs), 10,
                                f"guide_observation too short: {len(obs)} chars")
        self.assertLessEqual(len(obs), 300,
                             f"guide_observation too long: {len(obs)} chars")
    ...
    def test_guide_observation_no_color_tags(self):
        ...
        obs = result["guide_observation"]
        self.assertNotIn("<<COLOR:", obs,
                         f"Color tags in guide_observation: {obs}")
...
class ChartOutputTest(SimpleTestCase):
    ...
    def test_trace_builders_exist(self):
        ...
        for name in ("histogram_trace", "boxplot_trace", "scatter_trace",
                      "line_trace", "bar_trace", "heatmap_trace",
                      "control_chart_trace", "reference_line"):
...
class EducationQualityTest(SimpleTestCase):
    ...
    def test_title_minimum_length(self):
        ...
        for key, entry in EDUCATION_CONTENT.items():
            ...
            self.assertGreaterEqual(len(title), 15,
    ...
    def test_dl_structure(self):
        ...
        for key, entry in EDUCATION_CONTENT.items():
            ...
            self.assertIn("<dl>", content,
                          f"Education for {key} missing <dl> structure")
            self.assertIn("<dt>", content,
                          f"Education for {key} missing <dt> elements")
            self.assertIn("<dd>", content,
                          f"Education for {key} missing <dd> elements")
    ...
    def test_all_analyses_have_education(self):
        ...
        if missing:
            self.fail(f"Analyses missing education ({len(missing)}): {', '.join(missing[:10])}"
...
class EvidenceGradeTest(SimpleTestCase):
    ...
    def test_grade_values_valid(self):
        ...
        for p in (0.001, 0.01, 0.03, 0.05, 0.1, 0.5):
            ...
            self.assertIn(result["grade"], self.VALID_GRADES,
...
class BayesianShadowTest(SimpleTestCase):
    ...
    def test_bf10_is_numeric(self):
        ...
        if shadow is not None:
            self.assertIsInstance(shadow["bf10"], (int, float),
    ...
    def test_shadow_types_produce_output(self):
        ...
        for shadow_type, kwargs in cases:
            ...
            self.assertIsNotNone(shadow,
                                 f"Shadow type '{shadow_type}' returned None with valid inputs")
...
class WhatIfSchemaTest(SimpleTestCase):
    ...
    def test_parameter_fields(self):
        ...
        result = standardize_output({
            "summary": "test",
            "power_explorer": {
                "test_type": "ttest", "observed_n": 30,
                "cohens_d": 0.5, "alpha": 0.05,
            },
        }, "stats", "ttest")
    ...
    def test_tier1_has_parameters(self):
        ...
        self.assertGreater(len(wi["parameters"]), 0,
                           "Tier 1 what_if has empty parameters")
...
class IntegrationTest(SimpleTestCase):
    ...
    def test_cpk_full_pipeline(self):
        ...
        self.assertGreaterEqual(len(edu.get("content", "")), 200,
                                "Cpk education content too shallow")
        ...
        self.assertGreaterEqual(len(nar.get("verdict", "")), 10,
                                "Cpk verdict too short")
...
class RegistryStructureTest(SimpleTestCase):
    ...
    REQUIRED_KEYS = {
        "module", "category", "has_pvalue", "effect_type", "shadow_type",
        "what_if_tier", "has_narrative", "has_education", "has_charts",
    }

    VALID_SHADOW_TYPES = {
        None, "ttest", "ttest2", "paired", "anova", "correlation",
        "chi2", "proportion", "regression", "variance", "nonparametric",
    }

    VALID_MODULES = {
        "stats", "spc", "ml", "viz", "bayesian", "reliability",
        "simulation", "d_type", "causal", "drift", "anytime",
        "bayes_msa", "quality_econ", "pbs", "ishap",
    }
    ...
    def test_all_entries_have_required_keys(self):
        ...
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            ...
            self.assertFalse(missing,
                             f"{atype}/{aid} missing keys: {missing}")

    def test_shadow_types_valid(self):
        """shadow_type is None or one of the known types."""
        from agents_api.dsw.registry import ANALYSIS_REGISTRY
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            self.assertIn(entry["shadow_type"], self.VALID_SHADOW_TYPES,
    ...
    def test_what_if_tier_valid(self):
        ...
        from agents_api.dsw.registry import ANALYSIS_REGISTRY
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            self.assertIn(entry["what_if_tier"], {0, 1, 2},
    ...
    def test_module_names_valid(self):
        ...
        from agents_api.dsw.registry import ANALYSIS_REGISTRY
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            self.assertIn(entry["module"], self.VALID_MODULES,
    ...
    def test_boolean_fields_are_bool(self):
        ...
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            for field in bool_fields:
                self.assertIsInstance(entry[field], bool,
    ...
    def test_analysis_type_matches_key(self):
        ...
        core_types = {"stats", "spc", "ml", "viz", "bayesian", "reliability",
                      "simulation", "d_type"}
        for (atype, aid), entry in ANALYSIS_REGISTRY.items():
            if atype in core_types:
                self.assertEqual(atype, entry["module"],
                                 f"Key type '{atype}' != module '{entry['module']}' for {aid}")

    def test_registry_not_empty(self):
        """Registry has a substantial number of entries (200+)."""
        from agents_api.dsw.registry import ANALYSIS_REGISTRY
        self.assertGreaterEqual(len(ANALYSIS_REGISTRY), 200,
    ...
    def test_pvalue_analyses_have_effect_or_shadow(self):
        ...
        if total_pval > 0:
            ...
            self.assertGreater(pct_covered, 0.30,
...
class DispatchPipelineTest(SimpleTestCase):
    ...
    def test_ml_routing(self):
        ...
        result = _run_analysis('ml', 'classification', df,
                               {'target': 'target', 'features': ['f1', 'f2'],
                                'algorithm': 'rf', 'split': 20})
...
class SPCCapabilityPostProcessTest(SimpleTestCase):
    ...
    def _build_mock_capability_response(self):
        ...
        return {
            ...
            "what_if_data": {
                "type": "capability",
                "mean": 50.0, "std": 2.0, "n": 100,
                "current_lsl": 44.0, "current_usl": 56.0,
            },
        }
    ...
    def test_education_injected(self):
        ...
        self.assertIsNotNone(result.get("education"),
                             "Education not injected for spc/capability")
...
class BayesianEngineTest(SimpleTestCase):
    ...
    def test_bayes_ttest_pipeline(self):
        ...
        try:
            result = _run_analysis('bayesian', 'bayes_ttest', df,
                                   {'var1': 'group_a', 'var2': 'group_b'})
    ...
    def test_bayes_regression_pipeline(self):
        ...
        df = pd.DataFrame({'x': x, 'y': 2 * x + np.random.normal(0, 0.5, 50)})
        try:
            result = _run_analysis('bayesian', 'bayes_regression', df,
                                   {'target': 'y', 'features': ['x']})
...
class ReliabilityEngineTest(SimpleTestCase):
    ...
    def test_weibull_pipeline(self):
        ...
        try:
            result = _run_analysis('reliability', 'weibull', df,
    ...
    def test_kaplan_meier_pipeline(self):
        ...
        try:
            result = _run_analysis('reliability', 'kaplan_meier', df,
...
class VizEngineTest(SimpleTestCase):
    ...
    def test_boxplot_pipeline(self):
        ...
        result = _run_analysis('viz', 'boxplot', df,
                               {'var': 'value', 'by': 'group'})
...
class MLEngineTest(SimpleTestCase):
    ...
    def test_clustering_pipeline(self):
        ...
        result = _run_analysis('ml', 'clustering', df,
                               {'features': ['x', 'y'], 'n_clusters': 2})
...
class DTypeEngineTest(SimpleTestCase):
    ...
    def test_d_chart_pipeline(self):
        ...
        try:
            result = _run_analysis('d_type', 'd_chart', df,
                                   {'variable': 'measurement', 'factor': 'shift',
                                    'window_size': 50})
...
class StatsEngineBreadthTest(SimpleTestCase):
    ...
    def test_anova_pipeline(self):
        ...
        result = _run_analysis('stats', 'anova', df,
                               {'var1': 'value', 'groupby': 'group'})
    ...
    def test_chi_square_pipeline(self):
        ...
        result = _run_analysis('stats', 'chi_square', df,
    ...
    def test_correlation_pipeline(self):
        ...
        result = _run_analysis('stats', 'correlation', df,
                               {'var1': 'x', 'var2': 'y'})
    ...
    def test_mann_whitney_pipeline(self):
        ...
        np.random.seed(42)
        df = pd.DataFrame({
            'value': np.concatenate([np.random.normal(50, 5, 30),
                                     np.random.normal(55, 5, 30)]),
            'group': ['A'] * 30 + ['B'] * 30,
        })
        result = _run_analysis('stats', 'mann_whitney', df,
    ...
    def test_paired_ttest_pipeline(self):
        ...
        result = _run_analysis('stats', 'paired_ttest', df,
...
class SPCEngineBreadthTest(SimpleTestCase):
    ...
    def test_xbar_r_pipeline(self):
        ...
        result = _run_analysis('spc', 'xbar_r', df,
                               {'column': 'x', 'subgroup_column': 'subgroup'})
...
class FrontendContractTest(SimpleTestCase):
    ...
    def test_narrative_dict_renderable(self):
        ...
        from agents_api.dsw.standardize import standardize_output
        result = standardize_output({
            "summary": "A meaningful test summary for rendering."
        }, "stats", "ttest")
        nar = result["narrative"]
        for key in ("verdict", "body", "next_steps", "chart_guidance"):
            val = nar.get(key)
    ...
    def test_evidence_grade_is_string(self):
        ...
        if eg is not None:
            self.assertIsInstance(eg, str,
                                 f"evidence_grade is {type(eg)}, expected str")
    ...
    def test_education_dict_renderable(self):
        ...
        edu = result.get("education")
        if edu is not None:
            self.assertIsInstance(edu.get("title"), str,
                                 "education.title must be str")
            self.assertIsInstance(edu.get("content"), str,
                                 "education.content must be str")
    ...
    def test_diagnostics_have_renderable_keys(self):
        ...
        for diag in result.get("diagnostics", []):
            if isinstance(diag, dict):
                ...
                self.assertTrue(has_label or has_detail,
                                f"Diagnostic entry not renderable: {diag}")
            ...
            for p in wi["parameters"]:
                for num_key in ("min", "max", "step", "value"):
                    ...
                    self.assertFalse(math.isnan(val),
                                     f"what_if param '{p['name']}'.{num_key} is NaN")
                    self.assertFalse(math.isinf(val),
                                     f"what_if param '{p['name']}'.{num_key} is Inf")
