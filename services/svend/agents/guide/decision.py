...
class DecisionGuide(GuideAgent):
    ...
    def save_state(self) -> dict:
        """
        Save current interview state for persistence.

        Returns a dict that can be stored in database and restored later.
        """
        return {
    ...
    def load_state(self, saved: dict) -> None:
        """
        Restore interview state from saved data.

        Args:
            saved: Dict from save_state()
        """
        from .agent import InterviewState

        self.state = InterviewState(
            current_section=saved.get("current_section", 0),
            current_question=saved.get("current_question", 0),
        ...
        self.bias_warnings = [
            ...
            for w in saved.get("bias_warnings", [])
        ]
