...
class SchemaGenerator:
    ...
    DOMAIN_TEMPLATES = {
        ...
        "pricing": {
            ...
            "common_features": [
                FeatureSpec(
                    name="historical_avg_price",
                    feature_type=FeatureType.NUMERIC,
                    description="Average price over historical period",
                    constraints=NumericConstraints(min_value=0, distribution=DistributionType.LOGNORMAL),
                    importance="high",
                ),
                FeatureSpec(
                    ...
                    constraints=NumericConstraints(min_value=0, mean=0.15, std=0.1),
                    importance="high",
                ),
                FeatureSpec(
                    name="price_change_7d",
                    feature_type=FeatureType.NUMERIC,
                    description="Price change over last 7 days (%)",
                    constraints=NumericConstraints(min_value=-50, max_value=50, mean=0, std=5),
                    importance="high",
                ),
                FeatureSpec(
                    name="price_change_30d",
                    feature_type=FeatureType.NUMERIC,
                    description="Price change over last 30 days (%)",
                    constraints=NumericConstraints(min_value=-80, max_value=80, mean=0, std=10),
                    importance="high",
                ),
                FeatureSpec(
                    name="trading_volume",
                ...
                FeatureSpec(
                    ...
                    feature_type=FeatureType.NUMERIC,
                    description="Economic indicator index",
                    constraints=NumericConstraints(min_value=-100, max_value=100, mean=0, std=20),
                    importance="medium",
                ),
                FeatureSpec(
                    ...
                    constraints=NumericConstraints(min_value=0, max_value=365, mean=90, std=60),
                    importance="low",
                ),
            ],
            "edge_cases": [
                EdgeCase("market_crash", "Sudden market downturn", "rare", "Extreme volatility"),
                EdgeCase("low_liquidity", "Very low trading volume", "occasional", "Price may be unreliable"),
            ],
            "success_metrics": [
                SuccessMetric("accuracy", "Direction prediction accuracy", target_value=0.6, minimum_acceptable=0.52),
            ],
        },
    }
    ...
    def _detect_task_type(self, intent: str) -> str:
        """Detect whether intent is classification or regression."""
        import re
        intent_lower = intent.lower()

        # Regression indicators - predicting continuous values
        # Use word boundaries to avoid substring matches (e.g., "age" in "mortgage")
        regression_patterns = [
            r"\bprice\b", r"\bprices\b", r"\bcost\b", r"\bamount\b", r"\bvalue\b",
            r"\bsalary\b", r"\bincome\b", r"\brevenue\b", r"\bhow much\b",
            r"\bestimate\b", r"\bhousing price\b", r"\bhome price\b",
            r"\bproperty value\b", r"\bstock price\b", r"\btemperature\b",
            r"\brating\b", r"\bcount\b", r"\bnumber of\b", r"\bduration\b",
            r"\bweight\b", r"\bdistance\b", r"\bsize\b",
        ]

        # Classification indicators - predicting categories/outcomes
        classification_patterns = [
            r"\bchurn\b", r"\bfraud\b", r"\bdefault\b", r"\bdefaults\b",
            r"\bclassify\b", r"\bcategory\b", r"\bwill they\b", r"\bwhether\b",
            r"\byes or no\b", r"\bapprove\b", r"\breject\b", r"\bspam\b",
            r"\bmalicious\b", r"\blegitimate\b", r"\bconvert\b",
            r"\bdiagnose\b", r"\bdetect\b", r"\brisk level\b", r"\bforeclosure\b",
        ]
        ...
        # Default to classification (more common)
        return "classification"
    ...
    def _from_template(self, intent: str, domain: str) -> ProblemSchema:
        ...
        # First, detect if this is regression or classification from the intent
        task_type = self._detect_task_type(intent)

        # For regression tasks, override the template's classification defaults
        if task_type == "regression":
            # Determine regression target based on domain
            if domain == "mortgage":
                target_name = "property_price"
                target_desc = "Property/housing price to predict"
                target_type = FeatureType.NUMERIC
            elif domain == "pricing":
                target_name = "price"
                target_desc = "Price value to predict"
                target_type = FeatureType.NUMERIC
            elif domain == "healthcare":
                target_name = "outcome_score"
                target_desc = "Numeric outcome score"
                target_type = FeatureType.NUMERIC
            elif domain == "ecommerce":
                target_name = "purchase_amount"
                target_desc = "Purchase amount to predict"
                target_type = FeatureType.NUMERIC
            else:
                target_name = "value"
                target_desc = "Value to predict"
                target_type = FeatureType.NUMERIC

            return ProblemSchema(
                name=f"{domain.replace('_', ' ').title()} Price Predictor",
                intent=intent,
                domain=domain,
                ...
                target_classes=[],  # No classes for regression
                features=template["common_features"].copy(),
                edge_cases=template.get("edge_cases", []).copy() if template.get("edge_cases") else [],
                success_metrics=[
                    SuccessMetric("r2_score", "R² Score", target_value=0.7, minimum_acceptable=0.5),
                    SuccessMetric("rmse", "Root Mean Squared Error", target_value=None, minimum_acceptable=None),
                ],
                class_balance={},  # No class balance for regression
                assumptions=[
                    f"Features are representative of {domain} problems",
                    "This is a regression task predicting continuous values",
                    "Data distributions may differ from actual production data",
                ],
                data_to_collect=[
                    "Actual values from production",
                    "Feature values from real data",
        ...
        # Classification task - use domain-specific targets
        if domain == "churn":
            target_name = "churned"
            ...
            target_desc = "Whether price will go up or down"
            target_classes = ["down", "up"]
            class_balance = {"down": 0.5, "up": 0.5}
        else:
            target_name = "target"
