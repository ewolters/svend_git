"""PlantSim pull contract views — expose plant simulations for cross-tool integration."""
...
def _queryset(request):
    qs, _, _ = qms_queryset(PlantSimulation, request.user)
...
def _serialize(obj):
    return {
        "id": str(obj.id),
        "title": obj.name,
        "station_count": len(obj.stations or []),
...
def _get_container(request, pk):
    ...
    try:
        return qs.get(id=pk)
    except PlantSimulation.DoesNotExist:
        return None
...
def _get_sim_dict(request, artifact_id):
...
def _get_sim(request, artifact_id):
...
def artifact_detail(request, artifact_id):
    return pull_views.pull_artifact_detail(request, artifact_id, get_artifact_fn=_get_sim_dict)
