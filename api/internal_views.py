...
def _resolve_recipients(target):
    ...
    if target == "all":
        ...
        cutoff = now - timedelta(days=days)
        return (
            base.filter(Q(last_active_at__lt=cutoff) | Q(last_active_at__isnull=True)),
            None,
        )
...
def _markdown_to_html(text):
    ...
    html = re.sub(
        r"\[([^\]]+)\]\((https?://[^)]+)\)",
        r'<a href="\2" style="color:#4a9f6e;">\1</a>',
        html,
    )
    ...
    for user, email in recipients:
        ...
        if user:
            personalized = (
                ...
                .replace(
                    "{{firstname}}",
                    user.first_name or user.display_name or user.username,
                )
