"""Ishikawa pull contract views — expose fishbone diagrams for cross-tool integration."""
...
def _queryset(request):
    qs, _, _ = qms_queryset(IshikawaDiagram, request.user)
...
def _serialize(obj):
    return {
        "id": str(obj.id),
        "title": obj.title or obj.effect[:80],
        "status": obj.status,
...
def _sub_artifact(obj, key_path):
    """Retrieve a sub-artifact field from an IshikawaDiagram."""
