...
def run_simulation(df, analysis_id, config, user):
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Name) and node.id not in allowed_names:
                    result["summary"] = (
                        f"Error: Forbidden name '{node.id}' in formula. Allowed: variable names + math functions."
                    )
                    return result
                ...
                if isinstance(node, ast.Attribute):
                    result["summary"] = (
                        "Error: Attribute access not allowed. Use function names directly (e.g., sqrt(x) not np.sqrt(x))."
                    )
                    return result
    ...
    summary += "<<COLOR:title>>MONTE CARLO SIMULATION<</COLOR>>\n"
    summary += f"<<COLOR:accent>>{'=' * 70}<</COLOR>>\n\n"
    ...
    result["guide_observation"] = (
        f"Monte Carlo simulation ({n_iter:,} iterations): output mean={out_mean:.4f}, std={out_std:.4f}. Top driver: {sensitivity[0]['variable'] if sensitivity else 'N/A'}."
    )
    ...
    result["narrative"] = _narrative(
        ...
        + (
            f" <strong>{_mc_top['variable']}</strong> is the top driver (swing = {_mc_top['swing']:.4f})."
            if _mc_top
            else ""
        )
...
def _tolerance_stackup(df, config, result):
    ...
    shapes.append(
        {
            "type": "line",
            "x0": wc_nominal - wc_tolerance,
            "x1": wc_nominal - wc_tolerance,
            "y0": 0,
            "y1": 1,
            "yref": "paper",
            "line": {"color": "#dc5050", "width": 2, "dash": "dash"},
    ...
    shapes.append(
        {
            "type": "line",
            "x0": wc_nominal - rss_tolerance,
            "x1": wc_nominal - rss_tolerance,
            "y0": 0,
            "y1": 1,
            "yref": "paper",
    ...
    summary += "<<COLOR:highlight>>Method Comparison:<</COLOR>>\n"
    ...
    result["summary"] = summary
    result["guide_observation"] = (
        f"Tolerance stackup: {len(names)} dims, WC=\u00b1{wc_tolerance:.4f}, RSS=\u00b1{rss_tolerance:.4f}, MC std={mc_std:.4f}. Top contributor: {contributions[0]['name']} ({contributions[0]['pct']:.0f}%)."
    )
        ...
        contrib_list.append(
            {
                ...
                "variance_contrib": var_contributions[name],
                "pct": pct,
                "input_mean": means[name],
                "input_std": math.sqrt(variances[name]),
            }
        )
