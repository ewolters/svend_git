...
def _run_drift_report(df, config, result):
    ...
    result["guide_observation"] = (
        ...
        + (
            ...
            if n_high > 0
            else ("Minor drift — monitor closely." if n_mod > 0 else "No evidence of drift — model appears healthy.")
        )
    )
    ...
    colors = [
        (
            ...
            if pr["severity"] == "high"
                ...
                else ("rgba(150,150,150,0.5)" if pr["severity"] == "low" else "rgba(74,159,110,0.5)")
            )
        )
        for pr in psi_results
    ]
